...
public class SponsorCommercialCreateService implements AbstractCreateService<Sponsor, Commercial> {
	...
	public void validate(final Request<Commercial> request, final Commercial entity, final Errors errors) {
		...
		if (!errors.hasErrors("number")) {
			errors.state(request, number.length() <= 16 && number.length() >= 13, "number", "sponsor.commercial.form.error.number", number);
		}
		 ...
		 */
		if (!errors.hasErrors("number")) {
			errors.state(request, number.startsWith("4") || number.startsWith("5") || number.startsWith("3") || number.startsWith("6"), "number", "sponsor.commercial.form.error.invalidStart", number);
		}
		...
		if (!errors.hasErrors("number")) {
			...
			errors.state(request, result % 10 == 0, "number", "sponsor.commercial.form.error.numberInvalid", number);

		}

		if (!errors.hasErrors("cvv")) {
			errors.state(request, cvv.length() == 3, "cvv", "sponsor.commercial.form.error.cvv", cvv);
		}

		if (!errors.hasErrors("deadline")) {
			errors.state(request, Pattern.matches("[0-9]{2}/[0-9]{4}", deadline), "deadline", "sponsor.commercial.form.error.deadline", deadline);
		}

		if (!errors.hasErrors("deadline")) {
			String[] monthYear = deadline.split("/");
			Integer month = Integer.parseInt(monthYear[0]);
			errors.state(request, month >= 1 && month <= 12, "deadline", "sponsor.commercial.form.error.month", deadline);
		}
	}
